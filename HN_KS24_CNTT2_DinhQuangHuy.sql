...
-- Liệt kê các lần đặt phòng diễn ra trong năm 2024 (không yêu cầu nói rõ phòng nào)
select * from bookings
where year(check_in) = '2024';

-- Cho biết số lượng phòng của từng loại phòng
select room_type, COUNT(*) as total_room from rooms
group by room_type;
...
select g.guest_name, COUNT(b.booking_id) as total_booking
from guests g
...
select 
    ...
    r.price_per_day,
    (b.check_out - b.check_in) * r.price_per_day as revenue
from bookings b
join guests g on b.guest_id = g.guest_id
join rooms r on b.room_id = r.room_id;
...
select 
    r.room_type,
    SUM((b.check_out - b.check_in) * r.price_per_day) AS total_revenue
from bookings b
join rooms r on b.room_id = r.room_id
...
select r.room_type, COUNT(b.booking_id) as total_booking
from bookings b
join rooms r on b.room_id = r.room_id
group by r.room_type
